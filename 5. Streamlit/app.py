...
#####################################################################################################################
TAPT_path = "/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/TAPT_Model_Save"
element_path = "/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/element_klue_model_dict_(26.46)0.913.pt"
emotional_path = "/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/emotional_klue_model_dict_0920.pt"
style1_path = "/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/style.css"
style2_path = "/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/style2.css"
style3_path = "/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/style3.css"
# Pretrain 모델 불러오기
#####################################################################################################################
...
def emotional_predict(predict_sentence):
    ...
    emotional_dataset_another = pd.DataFrame({'content': predict_sentence, 'label' : 0 }, index = [0])

    emotional_another_test = emotional_Dataset(emotional_dataset_another,emotional_tokenizer, emotional_max_len)
    emotional_test_dataloader = torch.utils.data.DataLoader(emotional_another_test, batch_size=1, num_workers=0)
    
    emotional_model.eval()

    for input_ids, attention_mask_token, token_type_ids, label in emotional_test_dataloader:
...
def extract_word(text):
  ...
  first = emoticon_normalize(text, num_repeats = 0) # 이모티콘
...
# 3요소 판별 
def is_in_ele(ele_list, text, sidebar2, sidebar3):
    chatbot_output1 = predict(extract_word(text)) # 문장 전처리 후 3요소 예측 진행
    # 바차트 그래프 그리기 
    element_bar_chart(chatbot_output1, text, sidebar2, sidebar3)
...
# 3요소 바차트
def element_bar_chart(chatbot_output1, text, sidebar2, sidebar3):
    ...
    with sidebar3.container():
        st.markdown(""" <style> @import url('https://fonts.googleapis.com/css2?family=Do+Hyeon&display=swap');
                                .font2 {
                                font-size: 20px ; font-family: 'Do Hyeon', sans-serif ; color: #31333F ;
                                text-align : center ; } 
                                </style> """, unsafe_allow_html=True)
        st.markdown('<p class="font2">● 현재 입력 문장의 요소별 수치 ●</p>', unsafe_allow_html=True)
        st.markdown(""" <style> .font3 {
                                font-size: 15px ; font-family: 'Do Hyeon', sans-serif ; color: #31333F ;
                                text-align : center ;} 
                                </style> """, unsafe_allow_html=True)
        st.markdown(f'<p class="font3">{text}</p>', unsafe_allow_html=True)
    with sidebar2.container():
        st.pyplot(fig)

def wordcloud_funtion():
    okt= Okt() # OKt 객체 선언
    ...
    word_cloud2 =['배달 빠름','비위생','친절','재주문','배달 느림','느림','불친절','안먹어','비린내','머리카락','비린내' ]
    ...
    # 이미지 파일 불러오기
    flower_mask=np.array(Image.open('/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/flower.png'))

    wordcloud = WordCloud(
        font_path = '/Library/Fonts/BMJUA_otf.otf', # 한글 글씨체 설정
    ...
    #사이즈 설정 및 출력
    save_path = '/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/wordcloud.png'
    crop_path = '/Users/keonkookkim/Desktop/KKK/AIFFEL/Aiffel_HakerThonFinal_project/Streamlit/wordcloud2.png'
    ...
    croppedImage = image1.crop((100,100,500,500))
...
class chat():
    ...
    def __init__(self, emotional, text_list):
        message("안녕하세요 고객님!! 저는 리뷰 분석기 입니다.")
        # time.sleep(1)
        message("오늘 드신 음식의 리뷰를 작성해 주세요")
        # time.sleep(1)
    ...
    def chatting(self):
        ...
        with placeholder.container(): # 리뷰 버튼을 밑으로 내리기 위해 필요 
            ...
            for i in range(len(st.session_state['past'])):
                ...
                must_say = is_in_ele(text=bot_text, ele_list=ele_list, sidebar2 = sidebar2, sidebar3 = sidebar3) # 부족한 요소 확인
                if len(ele_list)!=0 :
                    ...
                    with placeholder3.container():
                        message('작성해 주셔서 감사합니다! 리뷰를 분석 중입니다.', key=str(i) + '_bot')
                    local_css(style3_path)
                    placeholder2.empty()
                    timer_sidebar.empty()
                    ...
                    for i in range (len(st.session_state['past'])):
                        ...
                        for j in split_text:
                            st.session_state.word_cloud.append(j.text)
                            nega_score, posi_score = emotional_check(j.text)
                            second = pd.DataFrame({'review':j.text , 'negative_score' : nega_score , 'positive_score' : posi_score}, index = [0])
                            self.emotional = pd.concat([self.emotional, second], ignore_index= True)
                    ...
                    while (ts != 0):
                        ...
                        if self.user_input == True:
                            ...
                            if (ts == 0) or (len(ele_list) == 0):
                                with placeholder3.container():
                                    message('작성해 주셔서 감사합니다! 리뷰를 분석 중입니다.', key = str(st.session_state['generated']) +'_bot')
                                local_css(style3_path)
                                placeholder2.empty()
                                timer_sidebar.empty()
